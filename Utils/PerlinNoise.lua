...
function module.new(coords:Vector3, seed:number, amplitude:number, offset)
	local x, y, z = coords.X, coords.Y, coords.Z
	
	local ab = math.noise(x,y,seed)
	local bc = math.noise(y,z,seed)
	local ac = math.noise(x,z,seed)

	local abc = Vector3.new(ab,bc,ac)

	--[[local ba = math.noise(y,x,seed)
	local cb = math.noise(z,y,seed)
	local ca = math.noise(z,x,seed)

	local cba = Vector3.new(ba,cb,ca)]]

	local abcVector = abc
	
	local noise = ((abcVector.X + abcVector.Y + abcVector.Z)/3+offset) * amplitude / 6

	return noise
end

function module.new2(coords,amplitude,octaves,persistence)
	coords = coords or {}
